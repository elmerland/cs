...
public class EarthquakeManager
{
	...
	public static class HeapLog
		implements ComparableIndex<HeapLog>
	{
		...
		/**
		 * The QueueLog object that corresponds to this HeapLog object.
		 */
		...
		implements Indexable
	{
		...
		/**
		 * The Earthquake object for the earthquake event that this QueueLog
		 * object represents.
		 */
		private Earthquake	earthquake;
		
		/**
		 * The index in the max heap array of the HeapLog object that
		 * corresponds to this QueueLog object.
		 */
	...
	public void queryEarthquakes()
	{
		...
		if (heap.heapsize() == 0)
		{
			StreamManager.printMagnitudeQuery(null);
		}
		else
		{
			...
			StreamManager.printMagnitudeQuery(eq);
		}
	}
	...
	private void addNewEarthquakes(Report report)
	{
		...
		// Add all earthquake objects that are older than the time of the last
		// earthquake report.
		for (Earthquake quake : earthquakeList)
		{
			if (quake.getTime() > lastEarthquakeAddedTime)
			{
				...
				QueueLog newEarthquakeLog = new QueueLog(quake, heap.heapsize());
	...
	{
		// Determine the cutoff time for old earthquakes.
	...
	private void initDataStructures()
	{
		...
		queue = new LQueue<QueueLog>() {
			...
			public void enqueue(QueueLog log)
			{
				...
				super.enqueue(log);
				StreamManager.printEarthquakeAdded(log.getEarthquake());
				
				// Report earthquake to watcher manager.
				EqSpatial.getWatcherManager().reportEarthquake(log.getEarthquake());
			}
		};
