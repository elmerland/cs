	...
	implements BufferPoolADT
{
	 ...
	 */
	public BufferPool(String fileName, int bufferNum, int bufferSize)
		throws Exception
	{
		...
		throws Exception
	{
		...
		{
			System.out.println("Error: Invalid byte range. size: " + bytes.length
				+ " pos: " + pos);
			throw new Exception("Invalid byte range");
		}
		return recursiveAccess(false, bytes, pos);
	}
	...
	/**
	 * Gets the string representation of the ID of the buffer at the specified
	 * index in the buffer pool.
	 *
	 * @param index
	 *            The index of the buffer.
	 * @return The ID of the buffer at the specified index.
	 */
	public String getBufferID(int index)
	{
		int id = buffers[index].getID();
		if (id == -1)
		{
			return "Empty";
		}
		...
		throws Exception
	{
		if (blocks <= 0)
		{
			System.out.println("Error: The number of blocks is invalid.");
			throw new Exception("Invalid number of blocks to increase size by.");
		}
		long newSize = fileSize + (blocks * BUFFER_SIZE);
		srcFile.setLength(newSize);
		...
		throws Exception
	{
		...
		return recursiveAccess(true, bytes, pos);
	}


	/**
	 * Recursively writes or gets bytes from buffers. This method is used to
	 * allow the access of byte arrays that span across multiple blocks.
	 *
	 * @param write
	 *            Determines if is getting bytes or writing them.
	 * @param bytes
	 *            The bytes to be written, or the bytes to be obtained.
	 * @param pos
	 *            The starting position of the byte array.
	 * @return True if all the accesses were successful.
	 * @throws Exception
	 *             The disk could not be accessed.
	 */
	private boolean recursiveAccess(boolean write, byte[] bytes, long pos)
		throws Exception
	{
		// Write the bytes block by block.
		int written = 0;
		int start = 0;
		...
		while (written < bytes.length)
		{
			...
			long endPos = startPos + (end - start);

			// Get starting and ending block.
			...
			{
				endPos = (startBlock + 1) * BUFFER_SIZE;
				end = (int)(endPos - pos);
			}

			// Get byte array to write to disk.
			byte[] temp = Arrays.copyOfRange(bytes, start, end);

			// Write bytes to disk.
			if (write)
			{
				setBytesInBlock(temp, startPos);
			}
			else
			{
				getBytesInBlock(temp, startPos);
				int index = 0;
			...
			// Update starting and ending byte position.
			written += (end - start);
			start = end;
			end = bytes.length;
		}
	 ...
	 * specified by 'pos'. This method assumes that the byte array will always
	 * fit in a single buffer block.
	 *
	 * @param bytes
	 *            The bytes to be written.
	 * @param pos
	 *            The starting position of the bytes.
	 * @return True if the bytes were written successfully.
	 * @throws Exception
	 *             The bytes could not be written.
	 */
	private boolean setBytesInBlock(byte[] bytes, long pos)
		throws Exception
	{
	...
	/**
	 * Takes the byte array given and reads the bytes from the file position
	 * specified by 'pos'. This method assumes that the byte array will always
	 * fit in a single buffer block.
	 *
	 * @param bytes
	 ...
	 * @param pos
	 *            The starting position of the byte array.
	 * @return True if the bytes where read.
	 * @throws Exception
	 *             The bytes could not be read.
	 */
	private boolean getBytesInBlock(byte[] bytes, long pos)
		throws Exception
	{
		// Move buffer object to top.
		getBuffer(pos);
	...
	private boolean isValidByteRange(int size, long pos)
	{
		...
		if (pos < 0)
		{
			System.out.println("Error: Starting position is invalid.");
			return false;
		}
		return true;
	}
