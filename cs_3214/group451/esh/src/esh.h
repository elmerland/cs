...
typedef struct built_in {
	char * func_name;
	void (* built_in_func)(struct esh_command * cmd);
} built_in;
...
struct esh_pipeline {
	...
	struct termios saved_tty_state;  /* The state of the terminal when this job
										was stopped after having been in 
										foreground */
};
...
/**
 * Gets a pointer to the jobs list.
 * @return  A pointer to a list structure.
 */
struct list/* <esh_pipeline> */ * get_jobs(void);

/**
 * Gets the pipeline structure that corresponds to the process group with the
 * specified jid.
 * @param  jid The jid of the process group.
 * @return     A pointer to the pipeline structure.
 */
struct esh_pipeline * get_job_from_jid(int jid);
 ...
 * Gets the pipeline structure that corresponds to the process group with the
 * specified pgrp.
 * @param  pgrp The pgrp of the process group.
 * @return      A pointer to the pipeline structure.
 */
struct esh_pipeline * get_job_from_pgrp(pid_t pgrp);

/**
 * Gets the command structure that corresponds to the process with the
 * specified pid.
 * @param  pid The pid of the process.
 * @return     A pointer the command structure.
 */
struct esh_command * get_cmd_from_pid(pid_t pid);

/* Adds a new job to the job list */
/**
 * Adds a new job to the jobs list. The SIGCHLD signal is blocked during
 * the execution of this function. This function will not unblock the SIGCHLD
 * if it was previously blocked.
 * @param new_job [description]
 */
void add_job(struct list_elem * new_job);

/**
 * Removed a job in the jobs list that has a matching pgrp.
 * The SIGCHLD signal is blocked during the execution of this function.
 * @param pgrp The pgrp of the job to be removed from the jobs list.
 * This function will not unblock the SIGCHLD if it was previously blocked.
 */
void remove_job(pid_t pgrp);

/**
 * Remove all the completed or killed jobs from the jobs list.
 * If print_job is set to true this function will also print any jobs
 * that were trimmed.
 * @param print_job Set to true to print any trimmed jobs.
 */
void trim_jobs_list(bool print_job);

/**
 * Get a pointer to the jobs list.
 * @return  A pointer to the jobs list.
 */
struct list * get_jobs_list(void);

/**
 * Gets a pointer to the current foreground job. If there is no foreground
 * job running, the pointer will be NULL.
 * @return  A pointer to the current foreground job.
 */
struct esh_pipeline * get_fg_job(void);

/**
 * Replaces the old pointer of the foreground job with the one specified in
 * parameter.
 * @param new_fg The new pointer to the foreground job.
 */
